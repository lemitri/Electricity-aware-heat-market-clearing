...
#            for t in time:
#                alpha_R_sequential[W_max,s,h,t] = alpha_scenario_kmeans[scenario_dic_kmeans[s][2],h,t]*(rho_heat[h] + rho_elec[h]*r_min[h])-spot_price_sequential_scenario[W_max,s,t]*r_min[h]
#        
#        
#        for h in CHP_sorted['ex']:
#            for t in time:
#                if spot_price_sequential_scenario[W_max,s,t] < alpha_scenario_kmeans[scenario_dic_kmeans[s][2],h,t]*rho_elec[h]:
#                    alpha_R_sequential[W_max,s,h,t] = alpha_scenario_kmeans[scenario_dic_kmeans[s][2],h,t]*(rho_heat[h] + rho_elec[h]*r_min[h])-spot_price_sequential_scenario[W_max,s,t]*r_min[h]
#        
#                else:
#                    alpha_R_sequential[W_max,s,h,t] =  spot_price_sequential_scenario[W_max,s,t]*rho_heat[h]/rho_elec[h]
#        
#        for h in heat_pump:  
#            for t in time:
#                alpha_R_sequential[W_max,s,h,t] =  spot_price_sequential_scenario[W_max,s,t]/COP[h]
#                
#        for h in heat_only:  
#            for t in time:
#                alpha_R_sequential[W_max,s,h,t] = alpha_scenario_kmeans[scenario_dic_kmeans[s][2],h,t]
...
#        self.data.line_end = line_end
        ...
        #self.data.spot_price = {t:spot_price_sequential_scenario[s0,t] for t in time}
...
#        self.data.pipe_maxflow = pipe_maxflow 
        ...
        # Cost parameters
#        self.data.alpha_heat = {(g,t):alpha_R_sequential[W_max,s0,g,t] for g in CHP+heat_only for t in time}       
...
#        heat_cost_sequential_scenario[W_max,s0] = sum(Q_sequential[W_max,g,t]*alpha_scenario_kmeans[scenario_dic_kmeans[s0][2],g,t] for g in heat_only for t in time) + sum(Q_sequential[W_max,g,t]*rho_heat[g]*alpha_scenario_kmeans[scenario_dic_kmeans[s0][2],g,t] for g in CHP for t in time) + sum(Q_sequential_scenario[W_max,s0,g,t]/COP[g]*spot_price_sequential_scenario[W_max,s0,t] for g in heat_pump for t in time) + sum(alpha_up_kmeans[scenario_dic_kmeans[s0][2],h,t]*Q_up_sequential_scenario[W_max,s0,h,t] for h in CHP+heat_only for t in time) - sum(alpha_down_kmeans[scenario_dic_kmeans[s0][2],h,t]*Q_down_sequential_scenario[W_max,s0,h,t] for h in CHP+heat_only for t in time) 
