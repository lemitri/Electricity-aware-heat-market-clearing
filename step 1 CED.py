...
#for W_max in [50*x for x in range(1,13)]:
for W_max in W_range:
    for s0 in scenario_kmeans:
        ...
        #heat_cost_CED_scenario[s0] = sum(alpha_scenario[scenario_dic[s0][2],g,t]*rho_heat[g]*Q_CED_scenario[s0,g,t] for g in CHP for t in time) + sum(alpha_scenario[scenario_dic[s0][2],g,t]*Q_CED_scenario[s0,g,t] for g in heat_only for t in time)
    
        #elec_cost_CED_scenario[s0] = sum(alpha_scenario[scenario_dic[s0][2],g,t]*P_CED_scenario[s0,g,t] for g in gen for t in time) + sum(alpha_scenario[scenario_dic[s0][2],g,t]*rho_elec[g]*P_CED_scenario[s0,g,t] for g in CHP for t in time)
    
        for t in time:
            #heat_price_CED_scenario[s0,t] = dispatch_CED.constraints.heat_balance[t].Pi
...
for W_max in W_range:
    for s0 in scenario_kmeans:
        ...
        # - sum(Q_CED_scenario[W_max,s0,g,t]/COP[g]*spot_price_CED_scenario[W_max,s0,t] for g in heat_pump for t in time)
...
#        spot_price_CED_std[W_max,t] = np.std([spot_price_CED_scenario[W_max,s,t] for s in scenario_kmeans])
