...
#        for h in CHP_sorted['ex']:
#            for t in time:                
#                self.constraints.ratio_MP[h,t].rhs = P_max[h,t] + P_0_max[h,t] - self.data.r_min[h]*Q_min[h,t]
#         
#                self.constraints.maxprod_MP[h,t].rhs = self.data.CHP_maxprod[h] - (P_min[h,t]+ P_0_min[h,t])*self.data.rho_elec[h] - self.data.rho_heat[h]*Q_min[h,t]
#                self.constraints.minprod_MP[h,t].rhs =  - Q_max[h,t]
#                                    
#        for h in heat_pump:
#            for t in time:
#         
#                self.constraints.maxprod_MP[h,t].rhs = HP_load_min[h,t]  
...
#        m = self.model
#        
#        # objective function = minimizing total heat production cost (HOW IS IT CALCULATED? total production cost minus revenues from electricity sales)
#            
#        m.setObjective(gb.quicksum(self.data.alpha[s,h,t]*self.data.rho_heat[h]*self.variables.Q[h,t] for s in scenario_kmeans for h in CHP+heat_only for t in time)/S_all_kmeans+self.variables.beta,   
#            gb.GRB.MINIMIZE)
